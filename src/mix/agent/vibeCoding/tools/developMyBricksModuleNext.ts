...
`
    ...
    async stream(params: any, context) {
      ...
      const processFiles = (
        ...
        while (i < allFiles.length) {
          ...
          if (file.language === 'write' || file.language === 'delete') {
            ...
            const result = config.onUpdate?.({ files: [{ fileName: file.fileName, content: file.content, language: file.language }] });
            if (result) {
            ...
            if (afterFile && afterFile.language === 'after' && file.isComplete && afterFile.isComplete) {
              const result = config.onUpdate?.({ files: [
      ...
      if (status === 'ing') {
        ...
        streamProcessedIndex = processFiles(files, streamProcessedIndex, true);
      ...
      if (status === 'complete') {
        ...
        processFiles(files, streamProcessedIndex, false);
