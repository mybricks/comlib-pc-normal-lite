...
const RULE_POPUP_REF = `
  <规则: popupRef使用规范>
    ...
    - 控制浮层显示/隐藏状态的变量必须维护在 store 中，使用PopupVisible装饰器，保证设计态下默认打开；
...
<工作流程>
  ...
  2. **设计态检查**：确认设计态是否正常展示，如果不正常也需要修复；
  3. **直接修复**：对于所有问题，必须通过 before/after 格式直接输出修复代码；

  注意：仅关注报错和Review规则相关的问题，其他问题在你这里不属于问题，不需要关注也不需要修复；
</工作流程>
