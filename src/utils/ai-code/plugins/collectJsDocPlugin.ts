 ...
 *   export const XXX = comRef(...)   → key 为 "XXX"
 *   export const XXX = popupRef(...) → key 为 "XXX"
 ...
 * - ExportNamedDeclaration → 从自身 leadingComments 取（export const XXX = comRef() 场景）
 *
...
function extractMybricksJSDocFromNode(
  ...
  } else if (grandParentNode?.type === "ExportNamedDeclaration") {
    // export const XXX = comRef() 场景：注释挂在 ExportNamedDeclaration 上
    comments = grandParentNode.leadingComments;
  } else {
...
export default function collectJsDocPlugin(
  ...
  return {
    visitor: {
       ...
       * 以及：export const XXX = comRef(...) / popupRef(...)
      ...
      VariableDeclarator(path: any) {
        try {
          ...
          // 如果是 export const XXX = comRef()，注释挂在祖父级 ExportNamedDeclaration 上
          const parentNode = path.parentPath?.node ?? null;
          const grandParentNode = path.parentPath?.parentPath?.node ?? null;
          const jsdoc = extractMybricksJSDocFromNode(node, parentNode, grandParentNode);
