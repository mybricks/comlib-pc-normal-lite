...
): { summary?: string; props?: Array<{ type?: string; name: string; description?: string }> } | null {
...
/**
 * 把 JSDoc 注释的「原始字符串」解析成结构化数据：summary + props 数组。
 *
 ...
 *
 * 返回：{ summary?, props? }；若既没有 summary 也没有任何 prop 则返回 null。
 */
function parseJSDocComment(raw: string): { summary?: string; props?: Array<{ type?: string; name: string; description?: string }> } | null {
  ...
  if (!summary && props.length === 0) return null;
